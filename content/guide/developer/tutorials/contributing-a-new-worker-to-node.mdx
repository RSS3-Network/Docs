...
- **Engine**
  ...
  - **Worker**: The transforming logic for specific ÐApps (the focus of this tutorial).
...
1. For integrating new decentralized applications (ÐApps), implement the transformation logic in the `decentralized/contract` directory.
...
### Analyze the ÐApp for Integration

To effectively integrate a new ÐApp, it's crucial to first immerse yourself in its functionality. Utilize both the mainnet and testnet (if available) to gain a comprehensive understanding of its core processes and relevant information.
