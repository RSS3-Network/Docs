...
A Node is slashed when its [demotion](/guide/concepts/fee-and-calculation/demotion) counter reaches 4.

## Consequences

A slashed Node faces many consequences.

### [Reliability Score](/guide/concepts/fee-and-calculation/reliability-score)

The Node’s score is set to 0. This effectively means that the Node will not receive any requests during the current Epoch.

### [Staking Pool](/guide/concepts/pool/staking-pool)

0.5% of tokens in the Node’s Staking Pool will be temporarily put into the Slashing Pool.

### [Operation Pool](/guide/concepts/pool/operation-pool)
...
20% of the slashed tokens will be awarded to reporters. When the slash is automatically triggered without a reporter, 20% of the slashed tokens will be reimbursed to all developers who paid [request fees](/guide/concepts/fee-and-calculation/request-fee) during the Epoch in which the slashing occurred.

## Donation

The remaining tokens will be donated to the Treasury and used for [Public Good](/guide/concepts/nodes/public-good-node) initiatives and clauses.
