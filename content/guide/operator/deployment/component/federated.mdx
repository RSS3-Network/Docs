...
There is a new section under `component` for Federated components, which looks like this:
...
endpoints:
  # the rest of the config...
  mastodon:
    # This URL must be publicly accessible to allow:
    # 1. External relay services to connect
    # 2. Federation message subscription using relay URLs
    # 3. Reliable message delivery from relay servers
    url: http://your.mastodon.relay
...
component:
  ...
  federated:
    ...
    - id: mastodon-core
      ...
      parameters:
        # `relay_url_list` is a list of ActivityPub relay URLs that the worker communicate with.
        # You may specify multiple relays to expand coverage. If empty, a default relay URL list will be used.
        relay_url_list:
          [
            "https://relay.fedi.buzz/instance/mastodon.social",
            "https://relay.intahnet.co.uk/inbox",
          ]
        # `port` specifies the local port for forwarding requests from other relay servers.
...
By adding a Federated component to your RSS3 Node, you can seamlessly integrate data from federated networks into the RSS3 ecosystem, expanding the coverage and utility of your Node.
